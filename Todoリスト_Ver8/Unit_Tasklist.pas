...
begin
  ...
  begin
    if FTasks[i].Completed then Continue; // 完了したタスクはスキップ

    // 最初にフィルター条件を適用するため、初期値をTrueに設定
    ShowTask := True;
    ...
    else if (ComboBox_Priority.ItemIndex > -1) and
            (IntToStr(FTasks[i].Priority) <> ComboBox_Priority.Items[ComboBox_Priority.ItemIndex]) then
      ShowTask := False;
    ...
    if ShowTask then
    begin
      StringGrid_Tasklist.RowCount := Row + 1;  // 行を追加
      StringGrid_Tasklist.Cells[0, Row] := '';  // 空のセルを入れる
...
begin
  ...
  // グリッド表示も数字で更新
  StringGrid_Tasklist.Cells[2, SelectedRow] := IntToStr(PriorityValue);
end;
